    ...
    if kcal_manquant > 0:
        ...
        if st.session_state.get("show_fill"):
            ...
            if sug_mode == "Mes aliments":
                # ── Logique multi-aliments intelligente ──────────────────────
                # Priorités : protéines d'abord si déficit prot > 20g, glucides ensuite, légumes pour compléter
                prot_gap = max(0, settings["objectif_prot"] - totaux["proteines"])
                gluc_gap = max(0, settings["objectif_gluc"] - totaux["glucides"])

                # Catégoriser les aliments de la base
                PROT_FOODS = ["Filet de poulet", "Thon", "Skyr", "oeuf", "fromage blanc", "saumon", "truite", "lentille"]
                GLUC_FOODS = ["riz", "pâtes", "patate", "lentille", "pain", "avoine", "flocon"]
                VEG_FOODS  = ["brocoli", "épinard", "chou", "haricot", "courgette", "carotte"]

                def food_cat(name):
                    nl = name.lower()
                    if any(k in nl for k in PROT_FOODS): return "prot"
                    if any(k in nl for k in GLUC_FOODS): return "gluc"
                    if any(k in nl for k in VEG_FOODS):  return "veg"
                    return "other"

                combos = []

                # Générer combos de 1, 2 ou 3 aliments
                food_items = [(n, d) for n, d in foods_db.items() if d["kcal"] > 0]

                # Combo 1 aliment — si déficit faible < 200 kcal
                if kcal_manquant < 200:
                    for fname, fdata in food_items:
                        g = round(kcal_manquant / fdata["kcal"] * 100)
                        if 20 <= g <= 400:
                            m = calc_macros(fdata, g)
                            combos.append({
                                "items": [{"nom": fname, "grammes": g, **m}],
                                "kcal_total": m["kcal"],
                                "prot_total": m["proteines"],
                                "score": abs(m["kcal"] - kcal_manquant)
                            })

                # Combo 2 aliments — 1 protéine + 1 féculent OU 1 protéine + 1 légume
                prot_foods = [(n, d) for n, d in food_items if food_cat(n) == "prot"]
                gluc_foods = [(n, d) for n, d in food_items if food_cat(n) == "gluc"]
                veg_foods  = [(n, d) for n, d in food_items if food_cat(n) == "veg"]

                for p_name, p_data in prot_foods[:4]:
                    # Allouer 50-60% des kcal manquantes à la protéine si déficit prot élevé
                    prot_share = 0.6 if prot_gap > 20 else 0.4
                    kcal_prot = kcal_manquant * prot_share
                    g_prot = round(kcal_prot / p_data["kcal"] * 100)
                    if not (30 <= g_prot <= 500): continue
                    m_prot = calc_macros(p_data, g_prot)
                    kcal_reste = kcal_manquant - m_prot["kcal"]
                    if kcal_reste < 30: continue

                    # Compléter avec féculent si déficit glucides
                    pool2 = gluc_foods if gluc_gap > 15 else veg_foods
                    if not pool2: pool2 = veg_foods + gluc_foods
                    for c_name, c_data in pool2[:4]:
                        if c_name == p_name: continue
                        g_comp = round(kcal_reste / c_data["kcal"] * 100)
                        if not (20 <= g_comp <= 500): continue
                        m_comp = calc_macros(c_data, g_comp)
                        total_kcal = m_prot["kcal"] + m_comp["kcal"]
                        if abs(total_kcal - kcal_manquant) / max(kcal_manquant, 1) > TOLERANCE: continue
                        combos.append({
                            "items": [
                                {"nom": p_name, "grammes": g_prot, **m_prot},
                                {"nom": c_name, "grammes": g_comp, **m_comp},
                            ],
                            "kcal_total": round(total_kcal),
                            "prot_total": round(m_prot["proteines"] + m_comp["proteines"], 1),
                            "score": abs(total_kcal - kcal_manquant) + (prot_gap - m_prot["proteines"]) * 2
                        })

                # Combo 3 aliments — protéine + féculent + légume
                for p_name, p_data in prot_foods[:3]:
                    g_prot = min(300, max(50, round(prot_gap * 100 / max(p_data["proteines"], 1))))
                    m_prot = calc_macros(p_data, g_prot)
                    kcal_r1 = kcal_manquant - m_prot["kcal"]
                    if kcal_r1 < 50: continue
                    for g_name, g_data in gluc_foods[:3]:
                        g_gluc = round(kcal_r1 * 0.6 / g_data["kcal"] * 100)
                        if not (20 <= g_gluc <= 300): continue
                        m_gluc = calc_macros(g_data, g_gluc)
                        kcal_r2 = kcal_r1 - m_gluc["kcal"]
                        if kcal_r2 < 20: continue
                        for v_name, v_data in veg_foods[:3]:
                            if v_name in (p_name, g_name): continue
                            g_veg = round(kcal_r2 / v_data["kcal"] * 100)
                            if not (20 <= g_veg <= 400): continue
                            m_veg = calc_macros(v_data, g_veg)
                            total_kcal = m_prot["kcal"] + m_gluc["kcal"] + m_veg["kcal"]
                            if abs(total_kcal - kcal_manquant) / max(kcal_manquant, 1) > TOLERANCE: continue
                            combos.append({
                                "items": [
                                    {"nom": p_name, "grammes": g_prot, **m_prot},
                                    {"nom": g_name, "grammes": g_gluc, **m_gluc},
                                    {"nom": v_name, "grammes": g_veg,  **m_veg},
                                ],
                                "kcal_total": round(total_kcal),
                                "prot_total": round(m_prot["proteines"]+m_gluc["proteines"]+m_veg["proteines"],1),
                                "score": abs(total_kcal - kcal_manquant)
                            })

                # Trier par score (plus proche des objectifs = meilleur)
                combos.sort(key=lambda x: x["score"])
                combos = combos[:5]

                if not combos:
                    st.info("Aucune combinaison trouvée. Essaie Claude AI 🤖.")
                for i, combo in enumerate(combos):
                    col_s, col_b = st.columns([4, 1])
                    with col_s:
                        items_html = "".join([
                            f"<span style='color:#888'>{it['nom']} <span style='color:#555'>({it['grammes']}g)</span></span><br>"
                            for it in combo["items"]
                        ])
                        st.markdown(f"""
                        <div class="food-item" style="flex-direction:column;align-items:flex-start;gap:4px">
                            {items_html}
                            <span style="font-size:0.72rem"><span style="color:#e8ff5a">{combo['kcal_total']} kcal</span>
                            · P <strong>{combo['prot_total']}g</strong></span>
                        </div>""", unsafe_allow_html=True)
                    with col_b:
                        if st.button("➕", key=f"fill_food_{i}"):
                            for it in combo["items"]:
                                entry = {k: it[k] for k in ("kcal","proteines","glucides","lipides","fibres")}
                                entry.update({"nom": it["nom"], "grammes": it["grammes"]})
                ...
                if not api_key:
                    st.warning("⚠️ Clé API manquante. Ajoute ANTHROPIC_API_KEY dans Settings > Secrets de ton app Streamlit Cloud.")
...
{{"suggestion":"Titre repas","description":"description courte","kcal":300,"proteines":25,"glucides":30,"lipides":8,"portions":"150g poulet + 100g riz + 200g brocoli"}}"""
                            resp = requests.post(
...
with tab5:
    ...
    if settings_sub == "🧮 Calculateur":
        ...
        if st.button("🧮 Calculer mes besoins", key="btn_calc"):
            ...
            # CRITIQUE : le bouton "Appliquer" est un rerun séparé,
            # prot_g/gluc_g/kcal_reel n'existent plus sans ça.
            st.session_state["_calc_result"] = {
                "kcal_reel": kcal_reel,
                "prot_g":    prot_g,
                "gluc_g":    gluc_g,
                "lip_g":     lip_g,
            ...
            # ── AFFICHAGE RÉSULTAT ────────────────────────────────
            # (affiché dans le bloc "calculer" — résultats stockés dans _calc_result)

    # Afficher résultats + bouton appliquer HORS du bloc "if calculer"
    # => fonctionne même quand on clique "Appliquer" (rerun séparé)
    if "_calc_result" in st.session_state:
        r = st.session_state["_calc_result"]
        _kr  = r["kcal_reel"]
        _pg  = r["prot_g"]
        _gg  = r["gluc_g"]
        _lg  = r["lip_g"]
        _obj = r["obj_key"]
        _pw  = r["poids"]
        _ratio_map = {"Maintien":1.6,"Seche":2.0,"Prise de masse":1.8,"Recomposition corporelle":2.2}
        _ratio = _ratio_map.get(_obj, 1.6)
        st.markdown(f"""
        <div style="background:#111;border:1px solid #222;border-radius:16px;padding:20px;margin:16px 0">
          ...
          <div style="display:flex;justify-content:space-between;margin-bottom:6px">
            ...
            <span style="font-family:Space Mono,monospace;color:#f0f0f0">{r["bmr"]} kcal</span>
          </div>
          <div style="display:flex;justify-content:space-between;margin-bottom:16px">
            <span style="color:#888;font-size:0.8rem">TDEE (dépense totale)</span>
            <span style="font-family:Space Mono,monospace;color:#f0f0f0">{r["tdee"]} kcal</span>
          </div>
          <div style="border-top:1px solid #1e1e1e;padding-top:16px;margin-bottom:16px">
            <div style="font-size:0.7rem;color:#555;margin-bottom:6px">{_obj.upper()} &nbsp;·&nbsp; {r["deficit_label"]}</div>
            <div style="font-family:Space Mono,monospace;font-size:2.8rem;font-weight:700;color:#e8ff5a;line-height:1">{_kr}</div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
            <div style="background:#1a1a1a;border-radius:10px;padding:12px;text-align:center">
              <div style="font-family:Space Mono,monospace;font-size:1.2rem;font-weight:700;color:#e8ff5a">{_pg}g</div>
              <div style="font-size:0.62rem;color:#555;text-transform:uppercase;letter-spacing:1px">💪 Protéines</div>
              <div style="font-size:0.6rem;color:#333;margin-top:2px">{_ratio}g/kg</div>
            </div>
            <div style="background:#1a1a1a;border-radius:10px;padding:12px;text-align:center">
              <div style="font-family:Space Mono,monospace;font-size:1.2rem;font-weight:700;color:#5a8fff">{_gg}g</div>
            ...
            <div style="background:#1a1a1a;border-radius:10px;padding:12px;text-align:center">
              <div style="font-family:Space Mono,monospace;font-size:1.2rem;font-weight:700;color:#ff8c5a">{_lg}g</div>
              <div style="font-size:0.62rem;color:#555;text-transform:uppercase;letter-spacing:1px">🥑 Lipides</div>
              <div style="font-size:0.6rem;color:#333;margin-top:2px">{round(_lg*9/_kr*100)}% kcal</div>
            </div>
          </div>
          <div style="margin-top:14px;padding:10px;background:#0d0d0d;border-radius:8px;font-size:0.72rem;color:#555;line-height:1.6">
            Ces valeurs sont des estimations (±10%). Ajuste apres 2-3 semaines selon tes resultats.
          </div>
        </div>
        """, unsafe_allow_html=True)

        if st.button("Appliquer ces objectifs", key="btn_apply_calc"):
            st.session_state["objectif_kcal"] = _kr
            st.session_state["objectif_prot"] = _pg
            st.session_state["objectif_gluc"] = _gg
            st.session_state["objectif_lip"]  = _lg
            st.success("Objectifs appliques aux jauges !")
            st.rerun()
