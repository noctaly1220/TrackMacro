...
DEFAULT_FOODS = {
    ...
    "Steak haché 5% cru":         {"kcal":137,"proteines":21.0,"glucides":0.0,"lipides":5.0,"fibres":0.0,"source":"Générique"},
    "Steak haché 10% cru":        {"kcal":176,"proteines":20.0,"glucides":0.0,"lipides":10.0,"fibres":0.0,"source":"Générique"},
    "Steak haché 15% cru":        {"kcal":209,"proteines":19.0,"glucides":0.0,"lipides":15.0,"fibres":0.0,"source":"Générique"},
    "Œufs entiers":               {"kcal":140,"proteines":12.7,"glucides":0.7,"lipides":9.8,"fibres":0.0,"source":"Générique"},
    "Skyr nature":                {"kcal":60,"proteines":10.0,"glucides":4.0,"lipides":0.2,"fibres":0.0,"source":"Générique"},
    "Fromage blanc 0%":           {"kcal":46,"proteines":8.0,"glucides":3.8,"lipides":0.2,"fibres":0.0,"source":"Générique"},
    "Flocons d'avoine":           {"kcal":370,"proteines":13.5,"glucides":58.7,"lipides":7.0,"fibres":10.0,"source":"Générique"},
    "Patate douce crue":          {"kcal":86,"proteines":1.6,"glucides":20.1,"lipides":0.1,"fibres":3.0,"source":"Générique"},
    "Huile d'olive":              {"kcal":900,"proteines":0.0,"glucides":0.0,"lipides":100.0,"fibres":0.0,"source":"Générique"},
}

GENERIC_FOODS = DEFAULT_FOODS.copy()
...
def search_openfoodfacts(query):
    """Recherche fiable : résultats génériques utiles + Open Food Facts filtré par pertinence."""
    q = (query or "").strip()
    if not q:
        return []
    ...
    def normalize(txt):
        txt = (txt or "").lower()
        repl = {"é":"e","è":"e","ê":"e","ë":"e","à":"a","â":"a","ù":"u","û":"u","î":"i","ï":"i","ô":"o","œ":"oe","'":" ","-":" "}
        for a,b in repl.items():
            txt = txt.replace(a,b)
        return " ".join(txt.split())

    nq = normalize(q)
    raw_tokens = [t for t in nq.split() if len(t) >= 2]
    synonymes = {
        "hache": ["hach", "boeuf", "steak"],
        "haché": ["hach", "boeuf", "steak"],
        "boeuf": ["boeuf", "bœuf", "steak"],
        "poulet": ["poulet", "filet", "blanc"],
        "riz": ["riz", "basmati"],
        "pates": ["pates", "pâtes", "pate"],
        "avoine": ["avoine", "flocon"],
        "skyr": ["skyr"],
    }
    tokens = set(raw_tokens)
    for t in list(raw_tokens):
        tokens.update(synonymes.get(t, []))
    tokens = [normalize(t) for t in tokens if len(normalize(t)) >= 2]
    ...
    def relevance(name):
        ...
        for t in tokens:
            if t in nn:
                score += 4 if t in raw_tokens else 2
        if raw_tokens and all(t in nn for t in raw_tokens):
            score += 8
        return score
    ...
    # 1) Résultats génériques d'abord : plus fiable pour steak, riz, poulet, œufs, etc.
    for name, data in GENERIC_FOODS.items():
        sc = relevance(name)
        if sc > 0:
            key = normalize(name)
            seen.add(key)
            results.append({"name": name, **data, "_score": sc + 20})

    # 2) Open Food Facts : uniquement les produits dont le nom correspond vraiment à la recherche.
    headers = {"User-Agent": "MacroTracker/1.0 (contact: streamlit-app)"}
    params = {
        "search_terms": q,
        "page_size": 24,
        "fields": "product_name,product_name_fr,product_name_en,nutriments,brands,categories_tags_fr",
    }
    for base_url in ["https://fr.openfoodfacts.org/api/v2/search", "https://world.openfoodfacts.org/api/v2/search"]:
        try:
    ...
    results.sort(key=lambda x: (-x.get("_score", 0), x["name"].lower()))
    cleaned = []
    for r in results[:10]:
        r.pop("_score", None)
        cleaned.append(r)
...
def meal_title(items, target_kcal):
...
def generate_smart_suggestions(foods_db, totals, settings, style="Auto"):
    ...
    # Si la journée est vide et qu'il manque 2000 kcal, on ne propose pas un plat absurde de 2000 kcal.
    # On vise un vrai repas réaliste. L'utilisateur peut ensuite relancer pour compléter la journée.
    if style == "Léger / sèche":
        target_kcal = min(kcal_gap, 620)
    elif style == "Rapide":
        target_kcal = min(kcal_gap, 700)
    elif style == "Riche en protéines":
        target_kcal = min(kcal_gap, 850)
    else:
        target_kcal = min(kcal_gap, 820)
    gaps = {
        "kcal": target_kcal,
    ...
    for tpl in templates:
        ...
        for foods_choice in product(*[pools[cat] for cat in tpl]):
            ...
            for grams_choice in product(*grids):
                ...
                if total["kcal"] < target_kcal * 0.60 and target_kcal > 300:
                    continue
                if total["kcal"] > target_kcal * 1.22 + 80:
                    continue
                kcal_diff = abs(total["kcal"] - target_kcal)
                ...
                veg_bonus = -15 if any(it["cat"] == "veg" for it in entries) and target_kcal > 350 else 0
                ...
                combo["titre"] = meal_title(entries, target_kcal)
...
# Garde les aliments ajoutés par l'utilisateur, mais réinjecte les bases utiles si elles manquent.
changed_defaults = False
for _fname, _fdata in DEFAULT_FOODS.items():
    if _fname not in foods_db:
        foods_db[_fname] = _fdata
        changed_defaults = True
if (not os.path.exists(DB_FILE)) or changed_defaults:
    ...
    if kcal_manquant > 0:
        ...
        if st.session_state.get("show_fill"):
            ...
            if sug_mode == "Suggestions intelligentes":
                ...
                if not combos:
                    st.markdown('<div class="notice">Je n’arrive pas à créer un plat cohérent avec les quantités actuelles. Essaie d’ajouter une protéine, un féculent, un légume et/ou une source de lipides dans la base, ou change le type de proposition.</div>', unsafe_allow_html=True)
