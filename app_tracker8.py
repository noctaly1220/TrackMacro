...
def apply_supabase_auth() -> None:
    """Re-applies the logged-in user JWT to the cached Supabase client after each Streamlit rerun."""
    if not USE_SUPABASE:
        return
...
def auth_gate() -> bool:
    init_local_state()
    if not USE_SUPABASE:
        st.sidebar.caption("Mode démo/local")
        st.session_state.setdefault("sb_user", {"id": "demo-user", "email": "demo@local"})
        return True

    if st.session_state.get("sb_user"):
        if st.session_state["sb_user"].get("id") != "demo-user" and not st.session_state.get("sb_access_token"):
            st.session_state.pop("sb_user", None)
            st.session_state.pop("sb_refresh_token", None)
            st.caption("Reconnecte-toi une fois pour activer la sauvegarde Supabase.")
            return False
        apply_supabase_auth()
        with st.sidebar:
    ...
    if action:
        ...
        try:
            ...
            if user:
                ...
                if not session:
                    st.info("Compte créé. Confirme ton email si Supabase le demande, puis reconnecte-toi.")
                    return False
                st.session_state["sb_access_token"] = getattr(session, "access_token", "")
                st.session_state["sb_refresh_token"] = getattr(session, "refresh_token", "")
                apply_supabase_auth()
...
def ensure_user_defaults(user_id: str) -> None:
    ...
    try:
        ...
        st.session_state["last_db_error"] = f"Initialisation Supabase : {exc}"
...
def load_settings(user_id: str) -> Dict[str, float]:
    if USE_SUPABASE and user_id != "demo-user":
        try:
            ...
            st.session_state["last_db_error"] = f"Chargement objectifs : {exc}"
...
def load_foods(user_id: str) -> Dict[str, Dict[str, Any]]:
    if USE_SUPABASE and user_id != "demo-user":
        try:
            ...
            st.session_state["last_db_error"] = f"Chargement aliments : {exc}"
...
def load_entries(user_id: str, start: Optional[str] = None, end: Optional[str] = None) -> List[Dict[str, Any]]:
    if USE_SUPABASE and user_id != "demo-user":
        try:
            ...
            st.session_state["last_db_error"] = f"Chargement journal : {exc}"
...
def load_meals(user_id: str) -> Dict[str, Dict[str, Any]]:
    if USE_SUPABASE and user_id != "demo-user":
        try:
            ...
            st.session_state["last_db_error"] = f"Chargement repas : {exc}"
...
def search_openfoodfacts(query: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    The old /cgi/search.pl endpoint can return 403 on some deployments.
    """
    params = {
        "search_terms": query,
        "page_size": 10,
    ...
    headers = {"User-Agent": "MacroTracker/1.0 (Streamlit app)"}
    try:
        ...
        st.session_state["last_off_error"] = str(exc)
        return [], "Open Food Facts n'a pas répondu. Essaie un autre mot ou ajoute l'aliment manuellement."
...
with tab_add:
    ...
    if kcal_gap > 50:
        ...
        if st.session_state.get("show_suggestions"):
            ...
            suggestions = build_suggestions(foods, settings, totals)
            if not suggestions:
                st.caption("Aucune combinaison trouvée avec ta base actuelle. Ajoute quelques aliments protéinés/féculents ou utilise la suggestion IA.")
        ...
        if not meals:
            ...
            for it in meal.get("items", []):
                if it.get("food") not in foods:
                    st.caption(f"{it.get('food')} n'est plus dans ta base.")
                    continue
